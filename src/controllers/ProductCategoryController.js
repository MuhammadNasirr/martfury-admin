import * as catRepo from "../repository/ProductCategoryRepository";
...
export const getPublishedCategoryPublic = async (req, res, next) => {
  //   console.log(req);
  const { page } = req.query;
  delete req.query.page;
  try {
    console.log(req.jwtPayload);
    let respo = await catRepo.getPublishedCategoryPublic(req.query);
    if (respo.status === "success") {
      if (respo.data.cats.length) res.status(200).json(respo);
      else {
        res.status(204).json(respo);
